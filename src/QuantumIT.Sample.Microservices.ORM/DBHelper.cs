...
namespace QuantumIT.Sample.Microservices.ORM
{
    public class DBHelper : IDBHelper
    {
        ...
        public string ConstructColumsForUpdateWithoutCOALESCE<T>(string[] ignoreColumns)
        {
            var updateProperties = GetParameterizableProperties<T>();
            if (ignoreColumns != null)
                return string.Join(",", updateProperties.Where(c => !ignoreColumns.Contains(c.Name)).Select(c => $"{c.Name}={ParameterPrefix + c.Name}"));

            return string.Join(",", updateProperties.Select(c => $"{c.Name}={c.Name})"));
        }
